...
from .emailutils import enviar_factura_por_email_con_adjunto
from .pdf_generator import PDFGenerador
...
def cliente_dashboard(request):
    paciente_prueba = get_object_or_404(Paciente, cedula=paciente_cedula) # Cambia esto por una cédula existente (o elimina el try/except)
...
def enviar_factura_view(request, factura_id):
    ...
    try:
        # Forzar el acceso a un campo del paciente para verificar si la relación está rota
        paciente_nombre = factura.paciente.nombre
    except ObjectDoesNotExist:
        messages.error(request, "Error: El paciente asociado a esta factura ha sido eliminado.")
        return HttpResponseRedirect(reverse("cliente_dashboard"))

    # 1. SEGURIDAD: VERIFICACIÓN DEL DUEÑO ELIMINADA.

    # 2. Determinar el correo de destino
    correo_destino = "malaver6@gmail.com" # Cambia esto por tu email (o elimina el try/except)
    ...
    pdf_buffer_encriptado = None

    try:
        # 1. Instanciar el Generador
        generador = PDFGenerador()
        
        # 2. Generar y Encriptar en un solo paso
        pdf_buffer_encriptado = generador.generar_factura_con_encriptacion(factura=factura)

    except (ValueError, Exception) as e:
        # Captura errores de cédula faltante o de encriptación
        messages.error(request, f"Error al generar/encriptar el PDF: {e}")
        return HttpResponseRedirect(reverse("cliente_dashboard"))


    # 3. ENVIAR EL CORREO con el buffer encriptado
    if enviar_factura_por_email_con_adjunto(
        factura=factura,
        pdf_buffer=pdf_buffer_encriptado, # Se pasa el buffer encriptado
        correo_destino=correo_destino
    ):
        messages.success(request, f"Factura N° {factura.id} enviada y protegida a {correo_destino}.")
    else:
